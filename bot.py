...
MAIN_MENU = make_kb([
    ...
    [("💰 Доход без увеличения клиентов", "menu_ewa")],
])
...
KB_EWA_ACTION = make_kb([
    [("Хочу попробовать — свяжитесь со мной", "ewa_contact")],
...
EWA_HOOK = (
    "Есть одна штука, о которой большинство массажистов даже не задумываются.\n\n"
    "У вас уже есть клиенты, которые вам доверяют. Они спрашивают у вас "
    "совета по здоровью, по уходу за собой, по восстановлению.\n\n"
    "А что если вы можете не просто советовать, но и зарабатывать "
    "на этих рекомендациях? Не впаривая, не навязывая, а предлагая то, "
    "что реально работает.\n\n"
    "Одна молодая дерзкая компания с продуктами для здоровья и красоты "
    "даёт такую возможность. Без вложений в аренду, без найма, без сложных схем."
)

EWA_DETAILS = (
    "Компания называется Ewa Product. Ей 3 года, российское производство, "
    "контрактное. Продукция для здоровья и красоты — качественная, "
    "красиво упакованная и, что важно, понятная. Не нужно быть "
    "нутрициологом, чтобы о ней рассказать.\n\n"
    "Как это работает: вы рекомендуете продукты своим клиентам "
    "(тем, кто уже вам доверяет), они покупают по вашей ссылке, "
    "вы получаете вознаграждение. А если кто-то из них тоже захочет "
    "рекомендовать — вы получаете процент и с их продаж.\n\n"
    "Это не вместо массажа, а вместе. Как дополнительный этаж "
    "к тому, что у вас уже есть.\n\n"
    "Компания молодая и амбициозная. Заходить на старте — "
    "больше перспектив, чем когда бренд уже всем знаком."
)

EWA_NO = "Ок, без проблем. Если передумаете — кнопка никуда не денется."

EWA_CONTACT = (
    "Отлично! Напишите мне лично, и мы обсудим, как начать:\n"
    "👉 https://max.ru/u/f9LHodD0cOLXYIS0FGVsXNAJeZY_Z_xasaGyMt0gWldwVzuJBxfXQky_xdc"
)

EWA_PRODUCTS = (
    "Заходите в мой канал, там я рассказываю про продукты, "
    "про компанию и про свой опыт:\n"
    "👉 https://max.ru/id910607569617_biz"
)
...
def build_result_text(u):
    ...
    if user_price < market_avg * 0.85:
        price_note = f"→ Ваш чек (~{user_price}₽) ниже среднего по {city_name} ({market_avg}₽). Потенциал: +{int((market_avg - user_price) * clients_count * 4)}₽/мес при повышении до рыночного"
    elif user_price > market_avg * 1.15:
        price_note = f"→ Ваш чек (~{user_price}₽) выше среднего по {city_name} ({market_avg}₽) — это сильная позиция"
    else:
        price_note = f"→ Ваш чек (~{user_price}₽) на уровне рынка для {city_name} ({market_avg}₽)"

    scenarios = {
        "salon-exit": {
            "title": "вы переросли салон",
            "desc": "Вы работаете в салоне, но чувствуете, что пора работать на себя.\nЭто нормальный и закономерный этап роста, и у вас есть для этого база.",
            "points": [
        ...
        "hybrid-exit": {
            ...
            "full": "В подробном разборе вы получите расчёт вашей «точки безопасного выхода»,\nкакие каналы добавить, и конкретный таймлайн (план по времени) перехода.",
        },
        "hybrid-grow": {
        ...
        "private-grow": {
            ...
            "full": "В подробном разборе вы увидите, какие каналы подключить именно в вашей ситуации,\nкак поднять чек, и план выхода на стабильный доход.",
        },
        "private-optimize": {
            "title": "оптимизация практики",
            "desc": "У вас всё хорошо. Остаётся только вопрос, как расти дальше,\nкогда есть всего одна пара рук?",
            "points": [
            ...
            "full": "В подробном разборе я покажу вам, где именно вы теряете деньги,\nкак перейти от «много работаю» к «умно зарабатываю».",
        },
    }
...
async def send_material(cid, mat_key):
    """Отправить материал после email или пропуска."""
    names = {
    ...
    mat = names.get(mat_key, "Материал")
    set_user(cid, state=None)
    await reply(cid,
        f"📎 «{mat}» — отправляю!\n\n"
...
async def on_callback(event: MessageCallback):
    ...
    if data == "go_site":
        await reply(cid,
            ...
            "Там — детальный разбор с расчётами под вашу ситуацию.",
            KB_BACK
        )
        return
