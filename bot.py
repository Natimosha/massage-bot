...
async def on_bot_started(event: BotStarted):
    ...
    chat_id = str(event.message.chat.id)  # ИСПРАВЛЕНО
...
async def on_message(event: MessageCreated):
    ...
    chat_id = str(event.message.chat.id)  # ИСПРАВЛЕНО
    user = await get_user(chat_id)
    text = (event.message.text or "").strip()  # ИСПРАВЛЕНО (было event.message.body.text)
    ...
    if user.get("state") == "diag_waiting_name":
        name = text.split()[0].capitalize()  # Берём первое слово, с большой буквы
...
async def on_callback(event: MessageCallback):
    ...
    chat_id = str(event.message.chat.id)  # ИСПРАВЛЕНО (было event.message.recipient.chat_id)
    ...
    if data.startswith("src_"):
        if data == "src_done":
            # Завершить сбор источников → показать результат
            pass  # Продолжим ниже
        else:
            # Добавить источник (для private/hybrid — множественный выбор)
            wm = user.get("work_mode", "private-only")
            if wm == "salon-only":
                # Одиночный выбор для салона
                await update_user(chat_id, client_sources=[data])
            else:
                # Множественный — добавляем к существующим
                sources = user.get("client_sources", [])
                if data not in sources:
                    sources.append(data)
                    await update_user(chat_id, client_sources=sources)
                # Для private/hybrid ждём "src_done"
                await send(
                    chat_id,
        ...
        user = await get_user(chat_id)  # Перечитываем обновлённые данные
    ...
    if data.startswith("mat_"):
        ...
        # Здесь в будущем можно отправлять файл через bot.send_file()
...
async def chain_worker():
    ...
    не пора ли отправить сообщение цепочки кому-то из пользователей.
    """
    while True:
        try:
