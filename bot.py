...
EWA_HOOK = (
    "Есть одна возможность, о которой большинство массажистов не задумываются.\n\n"
    "У вас уже есть то, что нужно:\n"
    "▫️ Клиенты, которые вам доверяют\n"
    "▫️ Авторитет в вопросах здоровья и ухода за собой\n"
    "▫️ Регулярное общение один на один\n\n"
    "Что если на этом можно дополнительно зарабатывать — без увеличения числа сеансов?\n\n"
    "Не впаривая, не навязывая — просто рекомендуя то, что вы сами считаете хорошим."
)

EWA_DETAILS = (
    "Ewa Product — молодая российская компания, продукты для здоровья и красоты.\n\n"
    "Как это работает:\n"
...
async def on_callback(event: MessageCallback):
    ...
    if data == "go_site":
        await reply(cid,
            "На сайте — расширенная версия диагностики.\n"
            "Там больше вопросов, зато на выходе — детальный персональный PDF-план "
            "с расчётами и пошаговыми действиями именно под вашу ситуацию.\n\n"
